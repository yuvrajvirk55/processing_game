...
void draw(){
  ...
  enemySpawn(); //spawns the enemys (based on clock)
  ...
  //clock, scoreboard and control display
  clock();
  scoreboard(); 
  displayControls();
}
...
void clock(){ //clock used to determine score and enemy spawning
//Note to self: song length is 12000 on the clock
  clock += 1;
  //println(clock); //print of the clock, useful for when you are adding enemys in the spawner
}
...
void displayControls(){
  //displays controls until the clock reaches 250, i.e. until encountering the first enemy
  if(clock < 250){
    textAlign(CENTER);
    textSize(35);
    fill(0);
    ...
    textSize(35);
    fill(0);
    text("Current score: "+clock/60, 200, 50);
  }
}
...
void enemySpawn(){ //spawns the enemys based on the clock (this part of the code is incredibly long)
/*
Notes to self: 
- floor is 525 (square) and 550 (spike)
- good clock distance for linked objects 12 (square) and 8 (spike)
...
*/
  ...
  //===============(END)===============
  if(clock > 11800){ //completed the game
    gameComplete();
  }
  //===============(12000 = 200s)===============
}
